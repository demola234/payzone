...
func main() {
	...
	if err != nil {
		log.Fatal().Msg("cannot connect to db: ", err)
	}
...
func runGateWayServer(configs utils.Config, store db.Store) {
	...
	if err != nil {
		log.Fatal().Msg().Msg("cannot create server")
	}
	...
	statikFS, err := fs.New()
	if err != nil {
		log.Fatal().Msg().Msg("cannot create statik file system")
	}
	...
	if err != nil {
		log.Fatal().Msg("cannot start HTTP listener: ", err)
	}

	log.Printf("starting HTTP server on %s", listener.Addr().String())
	err = http.Serve(listener, mux)
	if err != nil {
		log.Fatal().Msg("cannot start HTTP GateWay server: ", err)
	}
}
...
func runGRPCServer(configs utils.Config, store db.Store) {
	...
	if err != nil {
		log.Fatal().Msg("cannot start grpc listener: ", err)
	}
	log.Printf("starting gRPC server on %s", configs.GRPCServerAddress)
	err = gRPCServer.Serve(listener)
	if err != nil {
		log.Fatal().Msg("cannot start grpc server: ", err)
	}
}
...
func runGinServer(configs utils.Config, store db.Store) {
	...
	if err != nil {
		log.Fatal().Msg("cannot start http server: ", err)
	}
}
