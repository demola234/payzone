...
func createRandomAccountsForUser(t *testing.T, user Users) []Accounts {
	...
	// Generate only unique currencies
	currencies := make(map[string]bool)
	currencies[utils.RandomCurrency()] = true

	for len(currencies) < 4 {
		currencies[utils.RandomCurrency()] = true
	}
...
func TestListAccountsForUser(t *testing.T) {
	user := createRandomUser(t)
	accounts := createRandomAccountsForUser(t, user)
	...
	accounts2, err := testQueries.ListAccountsByOwner(context.Background(), arg)
	require.NoError(t, err)
	...
	accounts2, err = testQueries.ListAccountsForUser(context.Background(), arg)
	require.NoError(t, err)
	require.Len(t, accounts2, len(accounts))
