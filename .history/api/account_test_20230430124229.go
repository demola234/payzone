...
func TestListAccounts(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		accounts[i] = generateRandomAccount(int64(i))
	}
...
func TestDeleteAccount(t *testing.T){
	...
	testCases := []struct{
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					DeleteAccount(gomock.Any(), gomock.Eq(id)).
					Times(1).
					Return(nil)
			},
			checkResponse: func(t *testing.T, recoder *httptest.ResponseRecorder) {
...
func TestUpdateAccount(t *testing.T){
	const id int64 = 20
	account := generateRandomAccount(id)
	testCases := []struct{
		name string
		accountID int64
		updateAccount db.UpdateAccountParams
		buildStubs func(store *mockdb.MockStore)
		checkResponse func(t *testing.T, recoder *httptest.ResponseRecorder)
	}{
		{
			...
			accountID: id,
			updateAccount: db.UpdateAccountParams{
				Owner: sql.NullString{String: "updated", Valid: true},
			},
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					UpdateAccount(gomock.Any(), gomock.Eq(id), gomock.Eq(db.UpdateAccountParams{
						Owner: sql.NullString{String: "updated", Valid: true},
					})).
					Times(1).
					Return(account, nil)
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T){
			...
			data, err := json.Marshal(tc.updateAccount)
			require.NoError(t, err)

			url := fmt.Sprintf("/accounts/%d", tc.accountID)
			request, err := http.NewRequest(http.Meth, url, bytes.NewReader(data))
			require.NoError(t, err)
