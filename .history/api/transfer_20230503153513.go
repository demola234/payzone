...
func (server *Server) validAccount(ctx *gin.Context, accountName string, currency string) bool {
	account, err := server.store.GetAccount(ctx, accountName)
	if err != nil {
	...
	if account.Currency != currency {
		err := fmt.Errorf("account [%d] currency mismatch: %s vs %s", accountName, account.Currency, currency)
