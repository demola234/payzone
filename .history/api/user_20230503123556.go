...
func (server *Server) loginUser(ctx *gin.Context) {
	...
	if loginReq.Username != "" {
		getUserByEmail, err := server.getUserByEmail(loginReq.Username, ctx)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, errorResponse(err))
			return
		}
	...
	if loginReq.Email != "" {
		getUserByEmail, err := server.getUserByEmail(loginReq.Email, ctx)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, errorResponse(err))
			return
		}
	...
	err := utils.CheckPassword(loginReq.Password, getUserByEmail.HashedPassword)
