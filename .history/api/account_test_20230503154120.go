	...
	testCases := []struct {
...

		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
					GetAccount(gomock.Any(), gomock.Eq(accounts.Owner)).
					Return(accounts, nil).
					Times(1)
			},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationBearer, accounts.Owner, time.Minute)
			},
			checkResponse: func(t *testing.T, recoder *httptest.ResponseRecorder) {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
					GetAccount(gomock.Any(), gomock.Eq("invalid")).
					Return(db.Accounts{}, sql.ErrNoRows).
					Times(1)
			},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationBearer, "invalid", time.Minute)
			},
			checkResponse: func(t *testing.T, recoder *httptest.ResponseRecorder) {
...
func TestCreateAccount(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationBearer, account.Owner, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
...
func TestDeleteAccount(t *testing.T) {
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			url := fmt.Sprintf("/accounts/%s", tc.accounntCurrency)
