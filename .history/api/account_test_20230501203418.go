...
func TestGetUsersAccountsByOwnerRequest(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		// Generate a random account
		account := generateRandomAccountWithOwnersName(t, users.Username, currencies[i])
	}
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					ListAccountsByOwner(gomock.Any(), gomock.Eq(users.Username), gomock.Eq(1), gomock.Eq(5)).
					Times(1).
