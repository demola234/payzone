...
func (server *Server) loginUser(ctx *gin.Context) {
	...
	if loginReq.Username == "" && loginReq.Email == "" {
		...
		// check if username or email exists from the database
		if loginReq.Username != "" {
			_, err := server.store.GetUser(ctx, loginReq.Username)
			if err != nil {
		...
		if loginReq.Email != "" {
			_, err := server.store.GetUserByEmail(ctx, loginReq.Email)
			if err != nil {
				if err == sql.ErrNoRows {
					ctx.JSON(http.StatusNotFound, errorResponse(err))
					return
