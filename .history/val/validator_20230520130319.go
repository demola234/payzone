...
func ValidateFullName(fullName string) error {
	err := ValidateStringField(func ValidateFullName(fullName string) error {
		, 6, 50)
	if err != nil {
		return err
	}
	if !isValidFullName(func ValidateFullName(fullName string) error {
		) {
		return fmt.Errorf("fullname must not contain special characters")
	}
	return nil
}
