...
func main() {
	...
	privateKey, _ := wallet.GetKey().GetWallet()
	fmt.Println("USDC: ", address, addressP2WPKH, addressP2WPKHInP2SH, privateKey)
	...
	transferUsdc(address, toaddress, privateKey.GetKey().PrivateHex())
}

func transferUsdc(fromAccount string, toAccount string, privateKeys string) {
